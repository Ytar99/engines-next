...
import { STATUS_OPTIONS_ARRAY } from "@/lib/constants/order";
...
export const validateProduct = (data, isUpdate = false) => {
  ...
  if ("categories" in data) {
    if (!Array.isArray(data.categories)) {
      ...
      const invalidIds = data.categories.filter((id) => isNaN(Number(id)));
...
export const validateOrder = (data, isUpdate = false) => {
  ...
  if (!STATUS_OPTIONS_ARRAY.includes(data.status)) {
